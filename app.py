...
from models import Follows, Likes, db, connect_db, User, Message
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        g.user.email = form.username.data or g.user.email
...
def homepage():
    ...
    if g.user:
        messages = (db.session
                    .query(Message, Follows)
                    .filter(Follows.user_following_id == g.user.id, Message.user_id == Follows.user_being_followed_id)
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
